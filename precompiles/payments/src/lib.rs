...
use alloc::vec::Vec;
use frame_support::traits::tokens::asset_ops::AssetIdOf;
use core::marker::PhantomData;
use pallet_payments::{Call, Config};
// use pallet_assets::{Config, TransferFlags};
...
alloy::sol!("src/precompiles/IPayments.sol");
use IPayments::{IPaymentsCalls, IPaymentsEvents};
...
impl AssetIdExtractor for InlineAssetIdExtractor {
	type AssetId = u32; // To do: check if needed to use AssetIdOf
...
/// An ERC20 precompile.
pub struct Payments<Runtime, PrecompileConfig, Instance = ()> {
	_phantom: PhantomData<(Runtime, PrecompileConfig, Instance)>,
}

impl<Runtime, PrecompileConfig, Instance: 'static> Precompile for Payments<Runtime, PrecompileConfig, Instance>
where
	...
	<<PrecompileConfig as AssetPrecompileConfig>::AssetIdExtractor as AssetIdExtractor>::AssetId:
		Into<<Runtime as pallet_assets::Config>::AssetId>,
	Call<Runtime>: Into<<Runtime as pallet_revive::Config>::RuntimeCall>,
	alloy::primitives::U256: TryInto<<Runtime as Config>::Balance>,
...
{
	type T = Runtime;
	type Interface = IPayments::IPaymentsCalls;
	...
	alloy::primitives::U256: TryFrom<<Runtime as Config<Instance>>::Balance>,
{
	...
	fn to_balance(value: alloy::primitives::U256) -> Result<<Runtime as Config<Instance>>::Balance, Error> {
	...
	fn to_u256(value: <Runtime as Config<Instance>>::Balance) -> Result<alloy::primitives::U256, Error> {
	...
	fn total_supply(
		...
		let value = Self::to_u256(pallet_assets::Pallet::<Runtime, Instance>::total_issuance(asset_id))?;
	...
	fn balance_of(
		...
		let value = Self::to_u256(pallet_assets::Pallet::<Runtime, Instance>::balance(asset_id, account))?;
